...
2. **Load review skills**
   - Load `contributor:code-review` skill (unless filtered out)
   - Load `contributor:security-review` skill (unless filtered out)
   - Load `contributor:youth-safety-review` skill (unless filtered out)
...
- `contributor:code-review` - Structure, content quality, marketplace checks
- `contributor:security-review` - Credentials, file safety, injection checks
- `contributor:youth-safety-review` - Language, personal info, content checks
...
Then:
```
/contributor:update-skill <skill-name>  # Generate changelog entries
