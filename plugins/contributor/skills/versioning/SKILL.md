...
3. **Use the update-skill command** to help generate entries:
   ```
   /contributor:update-skill <skill-name>
   ```
...
When creating a new skill with `/contributor:create-skill`, the version IS set to 1.0.0. This is the only time a version is "manually" set, and it's done by the create-skill command.
...
| Command | Purpose |
|---------|---------|
| `/contributor:create-skill` | Creates new skill at 1.0.0 |
| `/contributor:update-skill` | Generates changelog entries (NO version bump) |
| `/contributor:validate-skill` | Validates structure (NO version bump) |
| `/contributor:review` | Runs code review (NO version bump) |
